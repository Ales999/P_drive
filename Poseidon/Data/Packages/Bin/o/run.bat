...
IF NOT DEFINED oxygenpath (GOTO std_path) ELSE (GOTO run_program)
...
IF NOT DEFINED oxygenpath (GOTO steam_path) ELSE (GOTO run_program)

:steam_path
For /F "Tokens=2* skip=2" %%A In ('REG QUERY "HKEY_CURRENT_USER\Software\bohemia interactive\oxygen 2" /v "PATH"') Do (set oxygenpath=%%B && SET appExe=Oxygen2.exe)
